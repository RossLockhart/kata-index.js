...
function isBiggerThan10(num) {
  ...
  } else return `Number ${num} is equal to 10`;
...
function readTrafficLight(lightColour) {
  if(lightColour === "red" || lightColour === "RED"){
    return "STOP!"
  } if(lightColour === "amber" || lightColour === "AMBER"){
    return "GET READY...";
  } else return "GO!"; // although leaving it hanging like this is risky as for undefined values will result in "go!"
...
function checkInfinitive(word) {
   word.endsWith("re", "ir", "er") /*come back to this one later; the only way i can think to do this inside of if statements and the like is long winded 
   and likley won't work e.g if(word.length -1)*/
...
function checkBatteryLevel(batteryLevel) {
  ...
  } if(batteryLevel >5 && batteryLevel < 99){
    return `Battery level: ${batteryLevel}%`;
  } else {
    retrun `Fully Charged :)`;
  }
